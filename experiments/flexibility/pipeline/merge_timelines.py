...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="fifo_policy",
        help="Policy name for output file naming",
    )
    parser.add_argument(
        "--iter", type=int, default=0, help="Iteration number for output file naming"
    )
...
def main():
    ...
    serial_fnames = sorted(glob("/tmp/timeline_*.csv"))
    mpi_fnames = sorted(glob("/tmp/mpi_timeline_*.csv"))

    for fnames, out_name in (
        (serial_fnames, f"timeline_{args.policy}_{args.iter}"),
        (mpi_fnames, f"mpi_timeline_{args.policy}_{args.iter}"),
    ):
        ...
        if rank == 0:
            ...
            final_df.to_csv(f"{args.log_dir}/{out_name}.csv", index=False)
