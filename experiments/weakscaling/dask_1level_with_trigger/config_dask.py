...
def get_scheduler_file() -> str:
    ...
    return os.path.join(os.getcwd(), "scheduler.json")
...
class DaskWorkerConfig:
    ...
    def get_single_core_tile_config(self) -> Dict:
        ...
        cpu_affinity_gpu = "list:" + ":".join([f"{st}-{st+7}" for st in self.start_cores])
    ...
    def get_two_ccs_config(self) -> Dict:
        ...
        """
        cpu_affinity = "list:" + ":".join([f"{st}-{st+3}:{st+4}-{st+7}" for st in self.start_cores])
    ...
    def get_four_ccs_config(self) -> Dict:
        ...
        cpu_affinity = "list:" + ":".join(
            [f"{st}-{st+1}:{st+2}-{st+3}:{st+4}-{st+5}:{st+6}-{st+7}" for st in self.start_cores]
        )
    ...
    def get_any_worker_config(self, nworkers: int = 64) -> Dict:
        ...
        cpu_affinity = "list:" + ":".join(
            [f"{st}" for st in self.all_cores[:nworkers]]
        )
...
def launch_scheduler(interface: str = "bond0", base_port: int = 8786, logger: Optional[logging.Logger] = None) -> subprocess.Popen:
    """
...
def launch_workers_with_config(config_dict: Dict, interface: str = "bond0", logger: Optional[logging.Logger] = None, one_worker_per_node: bool = False):
    """
    ...
    for worker_type, worker_opts in config_dict.items():
        ...
        if one_worker_per_node:
            ...
            if logger:
                logger.info(f"Launching {total_workers} {worker_type} workers (1 per node, {nthreads_per_worker} threads each)")
        else:
            ...
            if logger:
                logger.info(f"Launching {total_workers} {worker_type} workers ({ppn} per node, 1 thread each)")
