...
# Clean Python cache to ensure workers use latest code
rm -rf ${dirname}/__pycache__
    ...
    for SLEEPTIME_MS in 0 1 10 100 1000 60000; do
        for NUMTASKS in 10; do
            for CONCURRENT_WORKERS in 102; do  # Add more: 24 48 102
                ...
                EXE="python ${dirname}/test_dask.py --sleep-time=${SLEEPTIME_SEC} --num-tasks=${TOTAL_TASKS} --concurrent-workers=${CONCURRENT_WORKERS} --scheduler-mode=manual --track-timeline --trigger"
                echo ${EXE}
                ${EXE} 
                END_TIME=$(date +%s.%N)
