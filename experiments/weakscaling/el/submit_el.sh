...
source $HOME/.envs/el_dev2/bin/activate
        ...
        for NUMTASKS in 10; do
            for nlevels in 2; do
                for CONCURRENT_WORKERS in 102; do
                    ...
                    NSUCCESSFUL=0
                    if [ -f "main_status.json" ]; then
                        NSUCCESSFUL=$(jq '.nsuccessful_tasks' main_status.json)
                    fi
                    echo "Successful tasks: ${NSUCCESSFUL} / ${TOTAL_TASKS}"
                    if [ "${NSUCCESSFUL}" -ge "${TOTAL_TASKS}" ] 2>/dev/null; then
