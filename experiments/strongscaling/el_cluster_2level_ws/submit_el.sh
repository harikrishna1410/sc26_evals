...
source $HOME/.envs/el_pipeline/bin/activate
            ...
            for NUMTASKS in 1; do
                for nlevels in 2; do
                    for CONCURRENT_WORKERS in 102; do
                        ...
                        TOTAL_TASKS=$((NUMTASKS * 102 * 2048))

                        # Convert milliseconds to seconds for Python script
                        SLEEPTIME_SEC=$(echo "scale=6; ${SLEEPTIME_MS} / 1000" | bc)

                        ext=${CONCURRENT_WORKERS}_${NUMTASKS}_${SLEEPTIME_MS}ms_${nlevels}levels_${REQUEST_SIZE}_${i}
                        ...
                        EXE="python ${dirname}/test.py --sleep-time=${SLEEPTIME_SEC} --num-tasks=${TOTAL_TASKS} --concurrent-workers=${CONCURRENT_WORKERS} --nlevels=${nlevels} --request-size=${REQUEST_SIZE}"
                        echo ${EXE}
                        ${EXE}
                        END_TIME=$(date +%s.%N)
